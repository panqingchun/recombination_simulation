##### The script is to simulate the genotype of 200 MAGIC materials of 8 eight with DH method, and evaluate the number of recombination event and the size of recombinant fragments.
...
F2_12<-cbind(lilinone(M1)[,1:3],lilinone(M2)[,3:4]) #### F2 two parents cross#####
F2_34<-cbind(lilinone(M3)[,1:3],lilinone(M4)[,3:4]) #### F2 another two parents cross #####
F3_1234<-cbind(lilinone(F2_12)[,1:3],lilinone(F2_34)[,3:4]) #### F3 ####
F3_1234<-cbind(lilinone(F2_12)[,1:3],lilinone(F2_34)[,3:4]) #### F4 ####
F4_DH<-lilinone(F3_1234) ####  F4-DH ####
F4_DH
}
...
write.table("MAGIC-8parent-125k-DH-genotype.txt",col.names=T,row.names=F,sep="\t",quote=F) ###  the result file of genotype with 200 lines 
